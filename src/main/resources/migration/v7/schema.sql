...
SELECT 
    ...
    re.supplier AS fornecedor,
    re.pdf_order AS pedidoPDF,
    re.email as email_fornecedor,
    re.pdf_term AS termoPDF,
    re.empsiafi AS empSIAFI
...
-- CREATIONAL TESTING FIN BY IDS QUERY
select receivingid from tb_receiving where receivingid = 100;
select id_users from tb_users where id_users = 100;
select id_items from tb_items where id_items =100;
select id_details from tb_details where id_details= 100;
select id_cost_center from tb_cost_center where id_cost_center = 100;
select id_contact from tb_contact where id_contact =100;
